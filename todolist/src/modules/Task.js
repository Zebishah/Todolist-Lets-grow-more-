    ...
    return (
        <div className='flex flex-row gap-x-2 w-full justify-between p-2 rounded-lg shadow-sm shadow-black' key={props.index}>
            ...
            <div className='flex flex-row gap-x-4 justify-center items-center '>
                <FontAwesomeIcon icon={isChecked ? faXmark : faCheck} className='text-white font-custom font-bold text-lg cursor-pointer' onClick={check} />
                <FontAwesomeIcon icon={faTrash} className='text-white font-custom text-lg cursor-pointer ' onClick={() => props.delteTask(props.index)} />
            </div>
        </div>
    )
}
